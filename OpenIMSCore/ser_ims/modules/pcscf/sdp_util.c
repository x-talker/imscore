...
static char * send_rtpp_command(struct rtpp_node *node, struct iovec *v, int vcnt)
{
	...
	if (node->rn_umode == 0) {
		...
		if (len <= 0) {
			close(fd);
			LOG(L_ERR, "ERROR: send_rtpp_command: can't send command to a RTP proxy,errno:%d\n",errno);
			goto badproxy;
		}
		do {
		...
		for (i = 0; i < rtpproxy_retr; i++) {
			...
			if (len <= 0) {
				LOG(L_ERR, "ERROR: send_rtpp_command: "
				    "can't send command to a RTP proxy,errno:%d\n",errno);
				goto badproxy;
			}
...
int create_rtpp_command(struct sip_msg *  msg, char * m1p, char * m2p, char* c2p, char * v2p, char * c1p,
	...
	LOG(L_ERR, "ERROR: force_rtp_proxy2: oldip:%s\n",oldip);
	tmpstr1.s = m1p;
	tmpstr1.len = m2p - m1p;
...
int rtpproxy_child_init(int rank)
{
	...
	for (pnode = rtpp_list.rn_first; pnode != NULL; pnode = pnode->rn_next) {
		...
		LOG(L_INFO,"INFO:"M_NAME":mod:init pnode->rn_address:%s\n",pnode->rn_address);
		if (pnode->rn_umode == 0)
			goto rptest;
		/*
